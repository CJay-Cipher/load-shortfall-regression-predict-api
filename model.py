...
def _preprocess_data(data):
    ...
    def feature_engineering(clean_data):
        ...
        if "load_shortfall_3h" in df.columns:
            df = df.reindex(
                columns=[col for col in df.columns if col != 'load_shortfall_3h'] + 
                ['load_shortfall_3h'])
        
        return df

    df_train_new = feature_engineering(data_cleaning(feature_vector_df))
    # df_test_new = feature_engineering(data_cleaning(df_test))

    def feature_engr_2(data):
        new_df = data.copy()

        # pressure_cols = [col for col in new_df.columns if "pressure" in col]
        ...
        # new_df = new_df.drop(new_pressure_cols, axis=1)

        temp_cols = [col for col in new_df.columns if "temp" in col]
        ...
        for city in cities:   
            ...
            # new_df = new_df.drop(temp_list, axis=1)
        ...
        # new_df = new_df.drop(humidity, axis=1)
        ...
        # new_df = new_df.drop(wind_speed, axis=1)
        
        wind_deg = [column for column in new_df.columns if "wind_deg" in column]
        new_df["wind_deg_mean"] = new_df[wind_deg].mean(axis=1)
        # new_df = new_df.drop(wind_deg, axis=1)
        
        clouds_all = [column for column in new_df.columns if "cloud" in column]
        new_df["clouds_all_mean"] = new_df[clouds_all].mean(axis=1)
        # new_df = new_df.drop(clouds_all, axis=1)

        weather = [column for column in new_df.columns if "weather" in column]
        # new_df = new_df.drop(weather, axis=1)
        
        return new_df

    update_train = feature_engr_2(df_train_new)
    # update_test = feature_engr(df_test_new)
    ...
    df_new_train = standardiser(update_train)
    # df_new_test = standardiser(update_test)

    predict_vector = df_new_train[
       ...
       'hour', 'total_pressure_mean', 'Seville_temp_mean',
       'Bilbao_temp_mean', 'Madrid_temp_mean', 'Valencia_temp_mean',
       'Barcelona_temp_mean', 'humidity_mean', 'wind_speed_mean',
       'wind_deg_mean', 'clouds_all_mean']
    ]
    # ------------------------------------------------------------------------
